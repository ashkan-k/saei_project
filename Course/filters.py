...
class CourseFilters(filters.FilterSet):
    ...
    def status_filter(qs, name, value):
        qs.filter(status=True)
        return qs


class CourseUserFilters(filters.FilterSet):
    search = filters.CharFilter(method="search_filter")
    status = filters.CharFilter(method="status_filter")
    limit = filters.CharFilter(method="limit_filter")
    ...
    def search_filter(qs, name, value):
        qs = qs.filter(
            Q(course__teacher__user__first_name__icontains=value) |
            Q(course__teacher__user__last_name__icontains=value) |
            Q(course__teacher__user__national_id__icontains=value) |
            Q(course__teacher__user__father_name__icontains=value) |
            Q(course__teacher__user__phone__icontains=value) |
